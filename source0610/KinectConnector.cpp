...
void KinectConnector::setPoint(point_t set)
{
	pthread_mutex_lock(&mutex);
	point.x = set.x;
	point.y = set.y;
...
point_t KinectConnector::getPoint()
{
	pthread_mutex_lock(&mutex);
	point_t pt = point;
	pthread_mutex_unlock(&mutex);
...
void *KinectConnector::getGesture()
{
    try {
		...
		waveDetector.RegisterPointUpdate((void *)this, OnPointUpdate);
...
{
	KinectConnector* kinect  = (KinectConnector *)cxt;
	point_t pt ={pContext->ptPosition.X, pContext->ptPosition.Y, pContext->ptPosition.Z};
	kinect->setPoint(pt);
}
